...
import { CacheEntry, Episode, AnimeInfo, SearchResult, Pagination, FeaturedItem } from './types';
...
// Cache
const INFO_CACHE = new Map<string, CacheEntry>();
let LATEST_CACHE: { data: any; lastUpdate: number } = { data: null, lastUpdate: 0 };
let FEATURED_CACHE: { data: any; lastUpdate: number } = { data: null, lastUpdate: 0 };
...
app.get('/latest', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const scripts = $('script').map((i: number, el: any) => $(el).html()).get();
    const target = scripts.find((s: string) => s.includes('latestEpisodes'));
...
app.get('/featured', async (req: Request, res: Response) => {
  ...
  try {
    ...
    function extractString(objStr: string, key: string): string {
      const search = key + ':"';
      let pos = 0;
    ...
    function extractNumber(objStr: string, key: string): number {
      const search = key + ':';
      let pos = 0;
    ...
    function extractTopLevelString(objStr: string, key: string): string {
      const search = key + ':"';
      let pos = 0;
    ...
    function extractTopLevelNumber(objStr: string, key: string): number {
      const search = key + ':';
      let pos = 0;
    ...
    function findArrayEnd(str: string, arrayStartPos: number): number {
      let depth = 0;
      let inString = false;
      let escapeNext = false;
  ...
  try {
    ...
    $('article').each((i: number, el: any) => {
      ...
      if (a.length && img.length) {
        const labels = article.find('div').map((i: number, d: any) => $(d).text().trim()).get();
        resultados.push({
          titulo: h3.text().trim() || "Sin título",
          imagen: img.attr('src') || '',
          slug: (a.attr('href') || '').split('/media/')[1] || '',
          anio: labels.find((l: string) => /^\d{4}$/.test(l)) || "",
          tipo: labels.find((l: string) => ["TV", "MOVIE", "OVA", "SPECIAL"].includes(l)) || "Anime",
          cap: null
        });
      }
    });
    ...
    try {
      ...
      if (!totalPagesMatch && resultados.length >= 20) {
        pagination.totalPages = parseInt(page as string) + 1;
      }
    } catch (e: any) {
...
app.get('/anime-info', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const scripts = $('script').map((i: number, el: any) => $(el).html()).get().join('');
    ...
    const info: AnimeInfo = {
      ...
      generos: [...new Set([...scripts.matchAll(/name:"([^"]+)"/g)].map((m: RegExpExecArray) => m[1]))].filter((n: string) => n.length > 3 && !n.includes('Anime')).slice(0, 5),
      episodios: episodes.sort((a: Episode, b: Episode) => b.numero - a.numero)
    };
...
app.get('/get-video', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const scripts = $('script').map((i: number, el: any) => $(el).html()).get();
    const script = scripts.find((s: string) => s.includes('embeds'));
